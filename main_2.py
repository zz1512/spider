...
class DoubanSpider:
    ...
    def sort_films(self, films_list):
        ...
        return films_sorted
film_name = 'film_list.txt'
film_content = ''  # 最终要写到文件里的内容
film_content += '生成时间：' + time.asctime()

#film_content += "《%s》\t评分%s\t%s" % ( film_dic['film_name'], str(film_dic['points']), film_dic['film_stars'])
...
for film_dic in films_sorted:
    ...
    film_content += "\n《%s》\t评分%s\t%s\n'" % (film_dic['film_name'], str(film_dic['points']), film_dic['film_stars'])
    f = open(film_name,'w')
    f.write(film_content)
    #do sort_films (films_list)
    f.close()
